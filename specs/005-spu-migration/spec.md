...
### User Story 2 - SPU商品链接展示与购买 (Priority: P2)

作为小程序用户，我希望在SPU详情页的"商品链接"栏中看到该商品在拼多多的不同购买链接（含价格和规格），并可以一键跳转购买。

**Why this priority**: 商品链接栏是SPU详情页的核心购买转化入口。用户浏览SPU详情后，需要清晰的购买路径。当前仅支持拼多多平台数据，不做多平台价格对比。

**Note**: 多平台价格对比功能暂不实现（当前仅对接PDD API），未来扩展其他平台时可复用现有架构。

**Independent Test**: 可以独立测试：为某个SPU创建linked状态的spu_listings记录（含goods_sign），访问详情页"商品链接"栏，验证展示价格、规格、店铺信息，点击"去购买"后生成推广链接并跳转。

**Acceptance Scenarios**:

1. **Given** 用户进入SPU详情页并切换到"商品链接"Tab，**When** 该SPU有关联的linked listings时，**Then** 展示各链接的商品卡片（含店铺名、价格、销量、规格选择）
2. **Given** 用户查看商品链接列表，**When** 点击某个listing的"去购买"按钮时，**Then** 系统按需调用推广链接生成接口（Redis缓存1小时），返回短链后跳转至拼多多
...
- **FR-005**: 系统必须支持SPU详情页"商品链接"Tab，展示该SPU在拼多多的购买链接（含价格、店铺、规格、服务标签和购买入口）
- **FR-005a**: 系统按需生成推广链接：用户点击"去购买"时调用PDD API生成推广短链，Redis缓存1小时，PostgreSQL缓存12小时
- **FR-005b**: 系统必须支持推广链接的容错处理：goods_sign无效或商品下架时友好提示，不展示死链
...
- **Q**: 多平台价格对比功能是否实现？当前仅对接PDD API，如何处理？ → **A**: 多平台价格对比功能暂不实现。当前仅支持拼多多平台的商品链接展示和购买入口。US2已调整为"SPU商品链接展示与购买"，US2b合并至US2。未来扩展其他电商平台时可复用现有架构。
- **Q**: 商品链接栏需要展示哪些信息？ → **A**: 展示拼多多上该SPU的不同购买链接（含店铺名、价格、销量、规格选择、服务标签），用户点击"去购买"后生成推广链接跳转。不做跨平台比价。
